class Field {
  ...
  float posX;
  float posY;
  float fieldWidth; // save & load
  float noiseHeight; // save & load
  int arrayPosX; // save & load
  int arrayPosY; // save & load
  
  float oceanLevel = World.stdOceanLevel;  // save & load
  
  //// Energie- & Wachstumswerte
  final public static float maxOverallEnergy = 100;
  float energyValue = 0;  // save & load
  float maxEnergyValue;  // save & load 
  float regenerationrate;  // save & load
  float maxRegenerationrate = maxOverallEnergy/1000; 
  float[] influencingValues; 
  boolean influenceable;  // save & load
